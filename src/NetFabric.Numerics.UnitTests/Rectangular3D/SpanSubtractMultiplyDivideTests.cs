...
public class SpanVectorSubtractMultiplyDivideTests
{
    public static TheoryData<Vector<double>[], Vector<double>, Vector<double>[]> SubtractValueData
        => new()
        {
            ...
            },
            {
                ...
                new Vector<double>(3.0, 4.0, 5.0),
                new[] { new Vector<double>(-2.0, -2.0, -2.0), new Vector<double>(8.0, 8.0, 8.0) }
            },
            {
                Enumerable.Range(0, 97).Select(value => new Vector<double>(value + 0.5, (value * 2.0) - 10.0, value - 20.0)).ToArray(),
                new Vector<double>(1.25, -3.5, 2.0),
                Enumerable.Range(0, 97).Select(value => new Vector<double>((value + 0.5) - 1.25, ((value * 2.0) - 10.0) + 3.5, (value - 20.0) - 2.0)).ToArray()
            },
        };
    ...
    public static TheoryData<Vector<double>[], Vector<double>, Vector<double>[]> MultiplyValueData
        => new()
        {
            {
                Array.Empty<Vector<double>>(),
                new Vector<double>(2.0, 3.0, 4.0),
                Array.Empty<Vector<double>>()
            },
            {
                new[] { new Vector<double>(2.0, -3.0, 4.0) },
                new Vector<double>(-2.0, 0.5, -1.5),
                new[] { new Vector<double>(-4.0, -1.5, -6.0) }
            },
            {
                new[] { new Vector<double>(1.0, 2.0, 3.0), new Vector<double>(11.0, 12.0, 13.0) },
                new Vector<double>(3.0, 4.0, 5.0),
                new[] { new Vector<double>(3.0, 8.0, 15.0), new Vector<double>(33.0, 48.0, 65.0) }
            },
            {
                Enumerable.Range(0, 97).Select(value => new Vector<double>(value - 20.0, value + 4.0, value + 10.0)).ToArray(),
                new Vector<double>(-2.0, 0.25, -0.5),
                Enumerable.Range(0, 97).Select(value => new Vector<double>((value - 20.0) * -2.0, (value + 4.0) * 0.25, (value + 10.0) * -0.5)).ToArray()
            },
        };
    ...
    public static TheoryData<Vector<double>[], Vector<double>, Vector<double>[]> DivideValueData
        => new()
        {
            {
                Array.Empty<Vector<double>>(),
                new Vector<double>(2.0, 4.0, 5.0),
                Array.Empty<Vector<double>>()
            },
            {
                new[] { new Vector<double>(8.0, -6.0, 12.0) },
                new Vector<double>(2.0, -3.0, 4.0),
                new[] { new Vector<double>(4.0, 2.0, 3.0) }
            },
            {
                new[] { new Vector<double>(8.0, 10.0, 20.0), new Vector<double>(18.0, 24.0, 30.0) },
                new Vector<double>(2.0, 4.0, 5.0),
                new[] { new Vector<double>(4.0, 2.5, 4.0), new Vector<double>(9.0, 6.0, 6.0) }
            },
            {
                Enumerable.Range(0, 97).Select(value => new Vector<double>(value + 20.0, (value + 1.0) * 3.0, (value + 2.0) * 4.0)).ToArray(),
                new Vector<double>(2.0, 3.0, 4.0),
                Enumerable.Range(0, 97).Select(value => new Vector<double>((value + 20.0) / 2.0, value + 1.0, value + 2.0)).ToArray()
            },
        };
    ...
    {
        // arrange
        var source = new[] { new Vector<double>(10.0, 20.0, 30.0), new Vector<double>(40.0, 50.0, 60.0) };
        var value = new Vector<double>(2.0, 3.0, 4.0);
        var destination = new[] { new Vector<double>(1234.0, 5678.0, 9012.0) };
