...
| `model` | string | inherits parent | Single model string. When authoring agents, use the canonical lowercase CLI ID shown by GitHub's supported-models table or the CLI model picker (for example, `claude-sonnet-4.6`, `gpt-5.4`, or `gpt-5.3-codex`). Never emit a bare display label such as `GPT-5.4`. If compatibility requires display-name syntax, copy the complete picker value exactly, including any provider suffix (for example, `"GPT-5.4 (copilot)"`); canonical IDs remain preferred. When the session model is `Auto`, subagents always use the resolved session model regardless of this field. |
