...
| --- | --- | --- | --- | --- |
| GitHub Copilot CLI (`.agent.md`) | `model`, `reasoning-effort` | Single string only. Generate the canonical lowercase CLI ID from GitHub's current supported-models table or the CLI picker (for example, `gpt-5.4`, `gpt-5.3-codex`, `claude-sonnet-4.6`). Never generate a bare display label. If compatibility requires display-name syntax, copy the complete picker value exactly, including any provider suffix (for example, `"GPT-5.4 (copilot)"`); this is not the preferred authoring form. | Session model set to `Auto` overrides every agent's own `model:` field | None — an inline array is silently ignored. Approximate resilience via `~/.copilot/settings.json` → `subagents.agents.<name>.model` instead |
