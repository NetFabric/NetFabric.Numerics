...
[DebuggerDisplay("{Value}{UnitsSymbol}")]
...
public readonly record struct Angle<TUnits, T>(T Value) 
    : IEquatable<Angle<TUnits, T>>,
      ...
      //IMultiplyOperators<Degrees<TUnits, T>, Degrees<TUnits, T>, Degrees<TUnits, T>>,
...
{
    public static string UnitsSymbol => TUnits.Symbol;
    ...
    static Angle<TUnits, T> AdditiveIdentity
        => new(T.AdditiveIdentity);
    ...
    public static Angle<TUnits, T> operator +(Angle<TUnits, T> value) 
        => new(+value.Value);
